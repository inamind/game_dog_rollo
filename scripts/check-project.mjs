...
for (const file of [
  ...
  'image_sources/house_garden_updated.jpg',
]) {
...
assert(Object.values(OBJECTS).flat().every((item) => /^[\x20-\x7E]+$/.test(item.name)),
  '물건 표시 이름은 모두 영어여야 함');
...
assert(SPRITE_PATHS.explosion[0].endsWith('explosion_1.png')
  ...
  '필살기 폭발 프레임 순서 오류');
assert(LOCATIONS.yard.durationSeconds === 240, '마당 제한 시간은 4분이어야 함');
assert(LOCATIONS.house.durationSeconds === 300, '집 제한 시간은 5분이어야 함');
assert(GAME_FLOW.victoryFrameSeconds === 0.24, '승리 동작 프레임 시간 오류');
assert(GAME_FLOW.victoryRepeatCount === 3, '승리 동작 반복 횟수 오류');
assert(SPRITE_PATHS.victory.length === 3, '승리 동작 프레임 수 오류');

const indexSource = await readFile(join(root, 'index.html'), 'utf8');
assert(indexSource.includes('물건 15개 · 제한 시간 4분'), '마당 선택 화면 제한 시간 표시 오류');
assert(indexSource.includes('물건 20개 · 제한 시간 5분'), '집 선택 화면 제한 시간 표시 오류');
const stylesheetSource = await readFile(join(root, 'styles/main.css'), 'utf8');
assert(stylesheetSource.includes('url("../image_sources/house_garden_updated.jpg")'),
  '타이틀 화면 정원 배경 이미지 연결 오류');
assert(stylesheetSource.includes('background-size: cover'),
  '타이틀 화면 배경 전체 채우기 설정 오류');
...
levelupPlayer.setVictory();
assert(levelupPlayer.action === 'victory' && levelupPlayer.spritePath === SPRITE_PATHS.victory[0],
  '승리 동작 1프레임 시작 오류');
levelupPlayer.update(GAME_FLOW.victoryFrameSeconds);
assert(levelupPlayer.spritePath === SPRITE_PATHS.victory[1], '승리 동작 2프레임 전환 오류');
levelupPlayer.update(GAME_FLOW.victoryFrameSeconds);
assert(levelupPlayer.spritePath === SPRITE_PATHS.victory[2], '승리 동작 3프레임 전환 오류');
levelupPlayer.update(GAME_FLOW.victoryFrameSeconds);
assert(levelupPlayer.spritePath === SPRITE_PATHS.victory[0], '승리 동작 반복 오류');
